...
type lessonWithTopics struct {
	Lesson models.Lesson
	Topics []string
}

var tytLessonsAndTopics = []lessonWithTopics{
	{
		Lesson: analysis.Lesson{Name: "Türkçe", TotalQuestions: 40},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Tarih", TotalQuestions: 5},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Coğrafya", TotalQuestions: 5},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Felsefe", TotalQuestions: 5},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Din Kültürü", TotalQuestions: 5},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Matematik", TotalQuestions: 40},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Fizik", TotalQuestions: 7},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Kimya", TotalQuestions: 7},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Biyoloji", TotalQuestions: 6},
		Topics: []string{
...
var aytLessonsAndTopics = []lessonWithTopics{
	{
		Lesson: analysis.Lesson{Name: "Edebiyat", TotalQuestions: 24},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Tarih", TotalQuestions: 10},
		Topics: []string{"Tarih ve Zaman", "İnsanlığın İlk Dönemleri", "Orta Çağ’da Dünya", "İlk ve Orta Çağlarda Türk Dünyası",
			"İslam Medeniyetinin Doğuşu", "Türklerin İslamiyet’i Kabulü ve İlk Türk İslam Devletleri",
			"Yerleşme ve Devletleşme Sürecinde Selçuklu Türkiyesi", "Beylikten Devlete Osmanlı Siyaseti",
			"Devletleşme Sürecinde Savaşçılar ve Askerler", "Beylikten Devlete Osmanlı Medeniyeti",
			"Dünya Gücü Osmanlı", "Sultan ve Osmanlı Merkez Teşkilatı", "Klasik Çağda Osmanlı Toplum Düzeni",
	...
	{
		Lesson: analysis.Lesson{Name: "Coğrafya", TotalQuestions: 6},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Fizik", TotalQuestions: 14},
		Topics: []string{
	...
	{
		Lesson: analysis.Lesson{Name: "Kimya", TotalQuestions: 13},
		Topics: []string{"Kimya Bilimi", "Atom ve Periyodik Sistem", "Kimyasal Türler Arası Etkileşimler", "Kimyasal Hesaplamalar",
	...
	{
		Lesson: analysis.Lesson{Name: "Biyoloji", TotalQuestions: 13},
		Topics: []string{
...
func SeedData(db *mongo.Database) error {
	...
	tytLessonsCollection := db.Collection("tyt_lessons")
	tytTopicsCollection := db.Collection("tyt_topics")

	aytLessonsCollection := db.Collection("ayt_lessons")
	aytTopicsCollection := db.Collection("ayt_topics")

	for _, entry := range tytLessonsAndTopics {
		res, err := tytLessonsCollection.InsertOne(ctx, entry.Lesson)
		if err != nil {
			return err
		}

		lessonId := res.InsertedID.(bson.ObjectID)
		...
		for _, t := range entry.Topics {
			...
			_, err := tytTopicsCollection.InsertOne(ctx, topic)
			if err != nil {
				return err
			}
		}
	...
	for _, entry := range aytLessonsAndTopics {
		res, err := aytLessonsCollection.InsertOne(ctx, entry.Lesson)
		if err != nil {
			return err
		}
		...
		for _, t := range entry.Topics {
			...
			_, err := aytTopicsCollection.InsertOne(ctx, topic)
			if err != nil {
				return err
			}
		}
