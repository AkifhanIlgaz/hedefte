...
func (service TopicService) GetAll() ([]models.Topic, error) {
	...
	if err != nil {
		service.logger.Error("TYT topicleri veritabanından çekilemedi", zap.Error(err))
		return nil, err
	}
	defer func() {
	...
	if err := cursor.All(context.Background(), &topics); err != nil {
		service.logger.Error("TYT topicleri cursor'dan alınırken hata oluştu", zap.Error(err))
		return nil, err
	}

	service.logger.Info("TYT topicleri başarıyla getirildi", zap.Int("count", len(topics)))
	return topics, nil
}
